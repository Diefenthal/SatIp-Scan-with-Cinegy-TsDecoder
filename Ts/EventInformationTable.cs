...
namespace Cinegy.TsDecoder.Tables
{
    ...
    {
        public static string[] RunningStatusDescription =
        {
            "undefined", "not running", "Starts in a few seconds", "pausing", "running", "reserved for future use",
        ...
        public string StartTimeString
        {
            get
            {
                ...
                return string.Format("{3,00}-{4,00}-{5,00} {0:x}:{1:x}:{2:x}", (StartTime >> 16) & 0xFF,
                    (StartTime >> 8) & 0xFF, StartTime & 0xFF, y, m, d);
            }
        }

        public uint Duration { get; set; } // 24     uimsbf

        public string DurationString => string.Format("{0:x}:{1:x}:{2:x}", (Duration >> 16) & 0xFF,
