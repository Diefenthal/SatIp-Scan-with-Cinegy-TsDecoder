...
namespace Cinegy.TsDecoder.TransportStream
{
    public class OptionalPes
    {
        ...
        public byte
            PtsdtsIndicator { get; set; } //	2 	11 = both present, 01 is forbidden, 10 = only PTS, 00 = no PTS or DTS
    ...
    public class Pes
    {
        ...
        {
            PacketStartCodePrefix = (uint) ((packet.PesHeader.Payload[0] << 16) + (packet.PesHeader.Payload[1] << 8) +
                                            packet.PesHeader.Payload[2]);
            ...
            StreamId
        {
            get;
            set;
        } //	1 byte 	Examples: Audio streams (0xC0-0xDF), Video streams (0xE0-0xEF) [4][5][6][7]
            ...
            PesPacketLength
        {
            get;
            set;
        } //	2 bytes 	Specifies the number of bytes remaining in the packet after this field. Can be zero. If the PES packet length is set to zero, the PES packet can be of any length. A value of zero for the PES packet length can be used only when the PES packet payload is a video elementary stream.[8]
            ...
            OptionalPesHeader
        {
            get;
            set;
        } //	variable length (length >= 9) 	not present in case of Padding stream & Private stream 2 (navigation data)
