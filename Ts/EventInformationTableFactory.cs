...
namespace Cinegy.TsDecoder.Tables
{
    public class EventInformationTableFactory : TableFactory
    {
        ...
        {
            get => base.InProgressTable as EventInformationTable;
        ...
        public void AddPacket(TsPacket packet)
        {
            ...
            if (packet.PayloadUnitStartIndicator)
            {
                ...
                InProgressTable.OriginalNetworkId =
                    (ushort) ((packet.Payload[pos + 10] << 8) + packet.Payload[pos + 11]);
            }
            ...
            while (startOfNextField < transportStreamLoopEnd)
            {
                var item = new EventInformationItem
                {
                    ...
                    StartTime = ((ulong) Data[startOfNextField + 2] << 32) +
                                ((ulong) Data[startOfNextField + 3] << 24) +
                                ((ulong) Data[startOfNextField + 4] << 16) + ((ulong) Data[startOfNextField + 5] << 8) +
                                Data[startOfNextField + 6],
                    Duration = (uint) ((Data[startOfNextField + 7] << 16) + (Data[startOfNextField + 8] << 8) +
                                       Data[startOfNextField + 9]),
                    RunningStatus = (byte) ((Data[startOfNextField + 10] >> 5) & 0x07),
                    FreeCAMode = (Data[startOfNextField + 10] & 0x10) == 0x10,
                    DescriptorsLoopLength =
                        (ushort) (((Data[startOfNextField + 10] & 0x3) << 8) + Data[startOfNextField + 11])
                };
                ...
                if (endOfDescriptors > Data.Length)
                    throw new InvalidDataException(
                        "Descriptor data in Event Information is marked beyond available data");
