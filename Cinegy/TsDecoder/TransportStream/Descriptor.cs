...
namespace Cinegy.TsDecoder.TransportStream
{
    ...
    public class ServiceListDescriptor : Descriptor
    {
        ...
        public static string ServiceTypeDescription(byte serviceType)
        {
            if (serviceType <= 0x0C)
                ...
                return "reserved for future use";
            else if (serviceType >= 0x80 && serviceType <= 0xFE) return "user defined";
    ...
    public class ServiceDescriptor : Descriptor
    {
        ...
        public static string GetServiceTypeDescription(byte serviceType)
        {
            ...
            if (serviceType >= 0x20 || serviceType <= 0x7F) return "reserved for future use";
            if (serviceType >= 0x80 || serviceType <= 0xFE) return "user defined";
            if (serviceType >= 0x12 || serviceType <= 0x15) return "reserved for future use";
            return "unknown";
        }
    }
    ...
    {
        public static string[] PolarizationDescription =
            {"Linear-horizontal", "linear-vertical", "circular-left", "circular-right"};

        public static string[] RoleOffDescription = {"α = 0,35", "α = 0,25", "α = 0,20", "reserved"};
    ...
    {
        public static string[] BandwidthDescription =
        {
        ...
        public static string[] ConstellationDescription = {"QPSK", "16-QAM", "64-QAM", "reserved for future use"};

        public static string[] HierarchyInformationDescription =
        {
            "non-hierarchical", "α = 1", "α = 2", "α = 4", "reserved for future us", "reserved for future us",
            "reserved for future us", "reserved for future us"
        };

        public static string[] CodeRateDescription =
        {
            "1/2", "2/3", "3/4", "5/6", "7/8", "reserved for future us", "reserved for future us",
            "reserved for future us"
        };

        public static string[] GuardIntervalDescription = {"1/32", "1/16", "1/8", "1/4"};

        public static string[] TransmissionModeDescription =
    ...
    public class BouquetNameDescriptor : Descriptor
    {
        ...
        {
            var lastIndex = start + 2;
            if (DescriptorLength != 0) BouquetName = Encoding.UTF8.GetString(stream, lastIndex, DescriptorLength);
        }
    ...
    public class ContentDescriptor : Descriptor
    {
        public ContentDescriptor(byte[] stream, int start) : base(stream, start)
        {
            ...
            while (length > 0)
                try
                {
                    ...
                    lastindex++;
                    if (ContentTypes == null) ContentTypes = new Collection<ContentType>();
        ...
        public static string GetConentNibble2Description(byte contentnibble1, byte contentnibble2)
        {
            ...
            {
                case 0x0:
                    if (contentnibble2 >= 0x0 || contentnibble2 <= 0xF) return "undefined content";
                    break;
                case 0x1:
                ...
                case 0xB:
                    ...
                    break;
                case 0xF:
                    if (contentnibble2 >= 0x0 || contentnibble2 <= 0xF) return "user defined";
                    break;
            }
    ...
    public class PrivateDataSpecifierDescriptor : Descriptor
    {
        public PrivateDataSpecifierDescriptor(byte[] stream, int start) : base(stream, start)
        {
            ...
            try
            {
                ...
                {
                    DataSpecifier = (stream[lastIndex] << 24) + (stream[lastIndex + 1] << 16) +
                                    (stream[lastIndex + 2] << 8) + stream[lastIndex + 3];
                    lastIndex += 4;
                }
            }
            catch (IndexOutOfRangeException)
    ...
    public class DataBroadcastDescriptor : Descriptor
    {
        public DataBroadcastDescriptor(byte[] stream, int start) : base(stream, start)
        {
            ...
            try
            {
                if (DescriptorLength != 0)
                {
                    ...
                    var selectorLength = (int) stream[lastIndex];
                    lastIndex++;
                ...
                var textLength = (int) stream[lastIndex];
                lastIndex++;
        ...
        public int DataBroadcastId { get; }
    ...
    public class CountryAvailabilityDescriptor : Descriptor
    {
        ...
        public bool AvailabilityFlag { get; }
        public Collection<string> CountryCodes { get; }
    }
    ...
    public class CaIdentifierDescriptor : Descriptor
    {
        public CaIdentifierDescriptor(byte[] stream, int start) : base(stream, start)
        {
            ...
            try
            {
                ...
                for (var offset2 = lastindex; offset2 < lastindex + DescriptorLength - 1; offset2 += 2)
    ...
    {
        private readonly List<LogicalchannelnumberItem> logicalChannelNumbers = new List<LogicalchannelnumberItem>();

        public LcnDescriptor(byte[] stream, int start)
            : base(stream, start)
        {
            for (var idx = start + 2; idx < start + DescriptorLength - 1; idx += 4)
            {
                var lcnItem = new LogicalchannelnumberItem();
        ...
        public class LogicalchannelnumberItem
        {
            ...
            public ushort LogicalChannelNumber { get; set; }
        }
    }
