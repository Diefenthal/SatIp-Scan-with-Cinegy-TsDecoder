...
namespace Cinegy.TsDecoder.TransportStream
{
    public class TsDecoder
    {
        ...
        public void AddData(byte[] data)
        {
            ...
            foreach (var packet in tsPackets) AddPacket(packet);
        }

        public void AddPackets(IEnumerable<TsPacket> newPackets)
        {
            foreach (var newPacket in newPackets) AddPacket(newPacket);
        }
        ...
        public ServiceDescriptor GetServiceDescriptorForProgramNumber(int? programNumber)
        {
            ...
            var serviceDesc =
                serviceDescItem?.Descriptors?.SingleOrDefault(sd => sd as ServiceDescriptor != null) as
                    ServiceDescriptor;

            return serviceDesc;
        }

        public T GetDescriptorForProgramNumberByTag<T>(int? programNumber, int streamType, int descriptorTag)
            where T : class
        {
            if (programNumber == null) return null;
        ...
        private void _nitFactory_TableChangeDetected(object sender, TransportStreamEventArgs e)
        {
            ...
            lock (this)
            {
                ...
                if (fact == null) return;
                message =
                    $"NIT {e.TsPid} Refreshed: (Version {NetworkInformationTable?.VersionNumber}, Section {NetworkInformationTable?.SectionNumber})";
            }
