...
namespace Cinegy.TsDecoder.Tables
{
    public class SpliceInfoTableFactory : TableFactory
    {
        ...
        {
            get => base.InProgressTable as SpliceInfoTable;
        ...
        public void AddPacket(TsPacket packet)
        {
            ...
            InProgressTable.EncryptionAlgorithm =
                (byte) ((packet.Payload[InProgressTable.PointerField + 5] >> 1) & 0x3F);
            InProgressTable.PTSAdjustment =
                ((((ulong) packet.Payload[InProgressTable.PointerField + 5] >> 7) & 0x1) << 32) +
                ((ulong) packet.Payload[InProgressTable.PointerField + 6] << 24) +
                ((ulong) packet.Payload[InProgressTable.PointerField + 7] << 16) +
                ((ulong) packet.Payload[InProgressTable.PointerField + 8] << 8) +
                packet.Payload[InProgressTable.PointerField + 9];
            InProgressTable.CWIndex = packet.Payload[InProgressTable.PointerField + 10];
            InProgressTable.Tier = (ushort) ((packet.Payload[InProgressTable.PointerField + 11] << 4) +
                                             ((packet.Payload[InProgressTable.PointerField + 12] >> 4) & 0xF));
            InProgressTable.SpliceCommandLength =
                (ushort) (((packet.Payload[InProgressTable.PointerField + 12] & 0xF) << 8) +
                          packet.Payload[InProgressTable.PointerField + 13]);
            ...
            if (InProgressTable.SpliceCommandType == 5)
            {
                ...
                si.SpliceEventId = (ulong) ((packet.Payload[startOfNextField] << 24) +
                                            (packet.Payload[startOfNextField + 1] << 16) +
                                            (packet.Payload[startOfNextField + 2] << 8) +
                                            packet.Payload[startOfNextField + 3]);
                ...
                if (!si.SpliceEventCancelIndicator)
                {
                    si.OutOfNetworkIndicator = si.SpliceEventCancelIndicator =
                        (packet.Payload[startOfNextField + 5] & 0x80) == 0x80;
                    si.ProgramSpliceFlag = si.SpliceEventCancelIndicator =
                        (packet.Payload[startOfNextField + 5] & 0x40) == 0x40;
                    si.DurationFlag = si.SpliceEventCancelIndicator =
                        (packet.Payload[startOfNextField + 5] & 0x20) == 0x20;
                    si.SpliceImmediateFlag = si.SpliceEventCancelIndicator =
                        (packet.Payload[startOfNextField + 5] & 0x10) == 0x10;
                    if (si.ProgramSpliceFlag && !si.SpliceImmediateFlag)
                    {
                        ...
                        if (st.TimeSpecifiedFlag)
                            st.PTSTime = ((((ulong) packet.Payload[startOfNextField + 6] >> 7) & 0x1) << 32) +
                                         ((ulong) packet.Payload[startOfNextField + 7] << 24) +
                                         ((ulong) packet.Payload[startOfNextField + 8] << 16) +
                                         ((ulong) packet.Payload[startOfNextField + 9] << 8) +
                                         packet.Payload[startOfNextField + 10];
                    ...
                    else if (!si.ProgramSpliceFlag)
                    {
                        ...
                        startOfNextField += 7;
                        var components = new List<SpliceInsert.Component>();
                        for (var i = 0; i < si.ComponentCount; i++)
                        {
                            var component = new SpliceInsert.Component
                            {
                            ...
                            if (!si.SpliceImmediateFlag)
                            {
                                ...
                                if (st.TimeSpecifiedFlag)
                                {
                                    st.PTSTime = ((((ulong) packet.Payload[startOfNextField + 1] >> 7) & 0x1) << 32) +
                                                 ((ulong) packet.Payload[startOfNextField + 2] << 24) +
                                                 ((ulong) packet.Payload[startOfNextField + 3] << 16) +
                                                 ((ulong) packet.Payload[startOfNextField + 4] << 8) +
                                                 packet.Payload[startOfNextField + 5];
                                    startOfNextField += 5;
                                }
                    ...
                    if (si.DurationFlag)
                    {
                        ...
                        br.Duration = ((((ulong) packet.Payload[startOfNextField] >> 7) & 0x1) << 32) +
                                      ((ulong) packet.Payload[startOfNextField + 1] << 24) +
                                      ((ulong) packet.Payload[startOfNextField + 2] << 16) +
                                      ((ulong) packet.Payload[startOfNextField + 3] << 8) +
                                      packet.Payload[startOfNextField + 4];
                        startOfNextField += 5;
                    }

                    si.UniqueProgramId =
                        (ushort) ((packet.Payload[startOfNextField] << 8) + packet.Payload[startOfNextField + 1]);
