...
namespace Cinegy.TsDecoder.Buffers
{
    public class RingBuffer
    {
        ...
        /// <summary>
        ///     Returns the position of the ring-buffer indicating the array position of the next data will be entered into the
        ///     buffer
        /// </summary>
        ...
        private void ResetBuffers()
        {
            lock (_lockObj)
            {
                ...
                for (var n = 0; n <= BufferSize; ++n) _buffer[n] = new byte[_packetSize];
            }
        }
        ...
        public void Add(ref byte[] data, ulong timestamp)
        {
            lock (_lockObj)
            {
                if (BufferFullness == BufferSize) throw new OverflowException("Ringbuffer has overflowed");
                ...
                else
                {
                    throw new InvalidDataException(
                        "Data stored is greater than predefined maximum size (jumbo packet?)");
                }
            }
        }
        ...
        /// </summary>
        /// <returns>
        ///     Required size of reference buffer, if passed-in buffer was too small to accomodate data. Otherwise returns
        ///     zero.
        /// </returns>
        ...
        public int Remove(ref byte[] dataBuffer, out int dataLength, out ulong timestamp)
        {
            while (true)
            {
                lock (_lockObj)
                {
                    if (_lastRemPos > BufferSize) _lastRemPos = _lastRemPos % BufferSize - 1;
