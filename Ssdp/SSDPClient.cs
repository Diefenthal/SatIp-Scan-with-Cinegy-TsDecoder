...
namespace SatIp
{
    ...
    public class SSDPClient : IDisposable
    {
        ...
        private readonly Dictionary<string, SatIpDevice> _devices = new Dictionary<string, SatIpDevice>();
                ...
                if (u.Client != null)
                {
                    ...
                    if (notifyMatch.Success)
                    {
                        ...
                        if (!m.Success)
                            return;
                        var uuid = m.Value;
                        if (nts != null && nt != null && nt.Equals("urn:ses-com:device:SatIPServer:1") &&
                            nts.Equals("ssdp:byebye"))
                            if (usn != null)
                            {
                                if (_devices.ContainsKey(uuid)) _devices.Remove(uuid);
                                OnDeviceLost(new SatIpDeviceLostArgs(uuid));
                            }

                        if (nts != null && nt != null && nt.Equals("urn:ses-com:device:SatIPServer:1") &&
                            nts.Equals("ssdp:alive"))
        ...
        public void FindByType(string searchterm)
        {
            ...
            query.Append("\r\n");
            if (_unicastClient == null) _unicastClient = new UdpClient(_unicastPort);
            var req = Encoding.UTF8.GetBytes(query.ToString());
            var ipSsdp = IPAddress.Parse(_multicastIp);
        ...
        public SatIpDevice FindByUDN(string uuid)
        {
            ...
            if (_devices.ContainsKey(uuid)) _devices.TryGetValue(uuid, out device);
            return device;
        }
        ...
        {
            if (DeviceFound != null) DeviceFound(this, args);
        }

        protected void OnDeviceLost(SatIpDeviceLostArgs args)
        {
            if (DeviceLost != null) DeviceLost(this, args);
        }
