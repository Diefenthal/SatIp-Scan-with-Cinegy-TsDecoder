...
namespace SatIp
{
    public class RtspSession : INotifyPropertyChanged, IDisposable
    {
        ...
        private static readonly Regex RegexDescribeResponseSignalInfo = new Regex(@";tuner=\d+,(\d+),(\d+),(\d+),",
        ...
        /// </summary>
        private AutoResetEvent _keepAliveThreadStopEvent;
        ...
        public RtspSession(string serverAddress)
        {
        ...
        public string RtspStreamId
        {
            ...
            set
            {
                if (_rtspStreamId != value)
                {
                    _rtspStreamId = value;
                    OnPropertyChanged("RtspStreamId");
                }
            }
        }
        ...
        public int RtspSessionTimeToLive
        {
            ...
            set
            {
                if (_rtspSessionTimeToLive != value)
                {
                    _rtspSessionTimeToLive = value;
                    OnPropertyChanged("RtspSessionTimeToLive");
                }
            }
        }
        ...
        public int ServerRtpPort
        {
            ...
            set
            {
                if (_serverRtpPort != value)
                {
                    _serverRtpPort = value;
                    OnPropertyChanged("ServerRtpPort");
                }
            }
        }
        ...
        public int ServerRtcpPort
        {
            ...
            set
            {
                if (_serverRtcpPort != value)
                {
                    _serverRtcpPort = value;
                    OnPropertyChanged("ServerRtcpPort");
                }
            }
        }
        ...
        {
            get => _rtspSessionId;
            internal set => _rtspSessionId = value;
        }
        ...
        private void ProcessSessionHeader(string sessionHeader, string response)
        {
            if (!string.IsNullOrEmpty(sessionHeader))
            {
                ...
                if (!m.Success)
                    Logger.Error("Failed to tune, RTSP {0} response session header {1} format not recognised", response,
                        sessionHeader);
                _rtspSessionId = m.Groups[1].Captures[0].Value;
                _rtspSessionTimeToLive = m.Groups[3].Captures.Count == 1
                    ? int.Parse(m.Groups[3].Captures[0].Value)
                    : DefaultRtspSessionTimeout;
            }
        }
        ...
        private void ReceiveResponse(out RtspResponse response)
        {
            ...
            try
            {
                ...
                if (response.Headers.TryGetValue("Content-Length", out contentLengthString))
                {
                    ...
                    if (string.IsNullOrEmpty(response.Body) && contentLength > 0 ||
                        response.Body.Length < contentLength)
                    {
                        if (response.Body == null) response.Body = string.Empty;
                        while (responseBytesCount > 0 && response.Body.Length < contentLength)
                        {
                            responseBytesCount =
                                _rtspSocket.Receive(responseBytes, responseBytes.Length, SocketFlags.None);
                            response.Body += Encoding.UTF8.GetString(responseBytes, 0, responseBytesCount);
                        }
                    }
                }
            }
        ...
        {
            if (_keepAliveThread != null && !_keepAliveThread.IsAlive) StopKeepAliveThread();
            ...
            if (_keepAliveThread == null)
            {
                ...
                _keepAliveThread.Name = "SAT>IP tuner  keep-alive";
        ...
        private void KeepAlive()
        {
            try
            {
                while (!_keepAliveThreadStopEvent.WaitOne(RtspSessionTimeToLive)) // -5 seconds to avoid timeout
                {
                    ...
                    {
                        request = new RtspRequest(RtspMethod.Options,
                            string.Format("rtsp://{0}:{1}/", ServerAddress, 554), 1, 0);
                    }
                    else
                    {
                        request = new RtspRequest(RtspMethod.Options,
                    ...
                    if (response.StatusCode != RtspStatusCode.Ok)
                        Logger.Critical(
                            "SAT>IP : keep-alive request/response failed, non-OK RTSP OPTIONS status code {0} {1}",
        ...
        public RtspStatusCode Setup(string query, TransmissionMode transmissionmode)
        {
            ...
            if (string.IsNullOrEmpty(_rtspSessionId))
            {
                request = new RtspRequest(RtspMethod.Setup,
                    string.Format("rtsp://{0}:{1}/?{2}", ServerAddress, 554, query), 1, 0);
                switch (transmissionmode)
                {
                    case TransmissionMode.Multicast:
                        request.Headers.Add("Transport",
                            string.Format("RTP/AVP;{0}", transmissionmode.ToString().ToLower()));
                        break;
                    case TransmissionMode.Unicast:
                        ...
                        foreach (var connection in activeTcpConnections) usedPorts.Add(connection.LocalEndPoint.Port);
                        ...
                        request.Headers.Add("Transport",
                            string.Format("RTP/AVP;{0};client_port={1}-{2}", transmissionmode.ToString().ToLower(),
                                _rtpPort, _rtcpPort));
                        break;
                }
            }
            ...
            if (response.StatusCode == RtspStatusCode.Ok)
            {
                if (!response.Headers.TryGetValue("com.ses.streamID", out _rtspStreamId))
                    Logger.Error("Failed to tune, not able to locate Stream ID header in RTSP SETUP response");
                string sessionHeader;
                if (!response.Headers.TryGetValue("Session", out sessionHeader))
                    Logger.Error("Failed to tune, not able to locate Session header in RTSP SETUP response");
                ...
                if (!response.Headers.TryGetValue("Transport", out transportHeader))
                    Logger.Error("Failed to tune, not able to locate Transport header in RTSP SETUP response");
                ProcessTransportHeader(transportHeader);

                StartKeepAliveThread();
                _rtcpListener = new RtcpListener(Destination, RtcpPort, transmissionmode);
                _rtcpListener.PacketReceived += RtcpPacketReceived;
        ...
        public RtspStatusCode Play(string query)
        {
            ...
            if (string.IsNullOrEmpty(query))
                data = string.Format("rtsp://{0}:{1}/stream={2}", ServerAddress,
                    554, _rtspStreamId);
            else
                data = string.Format("rtsp://{0}:{1}/stream={2}?{3}", ServerAddress,
            ...
            string sessionHeader;
            if (!response.Headers.TryGetValue("Session", out sessionHeader))
                Logger.Error("Failed to tune, not able to locate Session header in RTSP Play response");
            ProcessSessionHeader(sessionHeader, "Play");
            string rtpinfoHeader;
            if (!response.Headers.TryGetValue("RTP-Info", out rtpinfoHeader))
                Logger.Error("Failed to tune, not able to locate Rtp-Info header in RTSP Play response");
            return response.StatusCode;
        }
        ...
        public RtspStatusCode Options()
        {
            ...
            if (string.IsNullOrEmpty(_rtspSessionId))
            {
                request = new RtspRequest(RtspMethod.Options, string.Format("rtsp://{0}:{1}/", ServerAddress, 554), 1,
                    0);
            }
            else
            {
            ...
            string sessionHeader;
            if (!response.Headers.TryGetValue("Session", out sessionHeader))
                Logger.Error("Failed to tune, not able to locate session header in RTSP Options response");
            ProcessSessionHeader(sessionHeader, "Options");
            string optionsHeader;
            if (!response.Headers.TryGetValue("Public", out optionsHeader))
                Logger.Error("Failed to tune, not able to Options header in RTSP Options response");
            return response.StatusCode;
        }
        ...
        public RtspStatusCode Describe()
        {
            ...
            if (string.IsNullOrEmpty(_rtspSessionId))
            {
                request = new RtspRequest(RtspMethod.Describe, string.Format("rtsp://{0}:{1}/", ServerAddress, 554), 1,
                    0);
                request.Headers.Add("Accept", "application/sdp");
            }
            else
            {
                request = new RtspRequest(RtspMethod.Describe,
                    string.Format("rtsp://{0}:{1}/stream={2}", ServerAddress, 554, _rtspStreamId), 1, 0);
                request.Headers.Add("Accept", "application/sdp");
                request.Headers.Add("Session", _rtspSessionId);
            }
            ...
            string sessionHeader;
            if (!response.Headers.TryGetValue("Session", out sessionHeader))
                Logger.Error("Failed to tune, not able to locate session header in RTSP Describe response");
        ...
        public RtspStatusCode TearDown()
        {
            if (_rtspSocket == null) Connect();
            RtspResponse response;
            var request = new RtspRequest(RtspMethod.Teardown,
                string.Format("rtsp://{0}:{1}/stream={2}", ServerAddress, 554, _rtspStreamId), 1, 0);
            request.Headers.Add("Session", _rtspSessionId);
            SendRequest(request);
            ReceiveResponse(out response);
        ...
        protected void RtcpPacketReceived(object sender, RtcpListener.RtcpPacketReceivedArgs e)
        {
            ...
            {
                var apppacket = (RtcpAppPacket) e.Packet;
